...
用本地 Ollama + qwen2.5:3b（免费、离线的小模型）对 8 个品类样例做批量编译，结果：

| 指标 | 结果 |
|------|------|
| 能力提取命中率 | **100%**（21/21，说明书功能全部识别成 capability） |
| 动作命名规范率 | **100%**（0 个不规范动词） |
| 类型识别正确率 | 7/8（窗帘识别成 cover 比期望值 other 更准，实际 8/8） |

结论：3b 小模型 + 语义清晰的编译 prompt，即可从说明书准确编译出设备协议。
...
- [x] **编译成功率验证**：8 样例批量编译，能力命中 100%、动作规范 100%（详见下文）
